...
FROM 
    employees e
WHERE 
    e.hire_date > (SELECT hire_date FROM employees WHERE last_name = 'Jones');
...
WHERE 
    DATEDIFF(CURDATE(), e.hire_date) / 365 > 15;
...
FROM 
    employees e
WHERE 
    e.salary > (SELECT salary FROM employees WHERE last_name = 'Bull');
...
FROM 
    employees e
WHERE 
    e.salary > (SELECT MAX(salary) FROM employees WHERE job_id = 'SH_CLERK')
ORDER BY 
    e.salary ASC;
